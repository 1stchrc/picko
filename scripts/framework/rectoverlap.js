var pickoRectOverlap = function(){
    var overlapedRects = new pickoMisc.ObjList();
    pickoRectOverlap = {
        ...
        GridLayer: function (sizeX,sizeY,scaleX,scaleY){
            ...
            for(var i = 0; i < this.gridList.length; i++){
                this.gridList[i] = new pickoMisc.ObjList();
            }
            
        }
    };
    pickoRectOverlap.GridLayer.prototype = {
        pushRect : function(rect,xi,xa,yi,ya){
            for(var y = yi; y < ya; y++){
                for(var x = xi; x < xa; x++){
        ...
        deleteRect : function(rect,xi,xa,yi,ya){
            for(var y = yi; y < ya; y++){
                for(var x = xi; x < xa; x++){
                    var set = this.gridList[x + y * this.sizeX];
                    set.remove(set.space.indexOf(rect));
                }
            }
        },
        ...
        updateRect : function(rect,l,b,r,t){
        ...
        overlapRect : function(l,b,r,t,callback){
            var xmin = Math.floor(l/this.scaleX);
            var xmax = Math.ceil(r/this.scaleX);
            var ymax = Math.ceil(t/this.scaleY);
            var ymin = Math.floor(b/this.scaleY);
            for(var y = ymin; y < ymax; y++){
                for(var x = xmin; x < xmax; x++){
                    this.gridList[x + y * this.sizeX].forEach(rect=>{
                        if(overlapedRects.space.indexOf(rect) != -1)
                            return;
                        if(rect.left > r)
                            return;
                        if(l > rect.right)
                        ...
                        if(b > rect.top)
                            return;
                        callback(rect);
                        overlapedRects.push(rect);
                    });
                }
            }
            overlapedRects.clear();
        }
    };
}
pickoRectOverlap();